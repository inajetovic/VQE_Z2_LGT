...
def main(n_plaquettes, n_layers, shots, iterations, J, m, mu):
    np.random.seed(0)
        ...
        for n_l in n_layers:
            for n_shots in shots:
                for n_iter in iterations:
                    print(f"Running: P: {n_plaq} L: {n_l} Ns: {n_shots} Ni: {n_iter}")
                    for n_j in J:
                        for n_m in m:
                            for n_mu in mu:
                                for flag in [False,True]:
                                    ...
                                    energies=run_vqe(n_plaq, n_l, n_j,n_m,n_mu,0,[],n_iter=n_iter,shots=n_shots,invariant=flag)
                                    new_row={
                                        "P":n_plaq,
                                        "L":n_l,
                                        "S":n_shots,
    ...
    csv_output_path = f"SPSA_{n_plaquettes}_layers_{n_layers}_shots_{n_shots}_iters_{iterations}_J_{J}_m_{m}_mu_{mu}.csv"
...
if __name__ == "__main__":
    ...
    main(args.n_plaquettes, args.n_layers, args.shots, args.iterations, args.J, args.m, args.mu)

#python spsa_shots_script_multiparams.py --n_plaquettes 1 2  --n_layers 1 2 3 --shots 1000 --iterations 200 --J 0 .01 1 5 10 --m .01 1 5 10 --mu .01 1 5 10